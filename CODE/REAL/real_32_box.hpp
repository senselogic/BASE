...
struct REAL_32_BOX
{
    ...
    REAL_32_BOX(
        ...
        IsEmpty( true ),
        MinimumPositionVector( 0.0f, 0.0f, 0.0f ),
        MaximumPositionVector( 0.0f, 0.0f, 0.0f )
    {
    }

    // -- INQUIRIES

    const REAL_32 GetWidth(
        ) const
    {
        DEBUG_Check( !IsEmpty );
    ...
    VOID GetCenterVector(
        REAL_32_VECTOR_3 & center_vector
        ) const
    {
        DEBUG_Check( !IsEmpty );
    ...
    VOID GetHalfSizeVector(
        REAL_32_VECTOR_3 & half_size_vector
        ) const
    {
        DEBUG_Check( !IsEmpty );

        half_size_vector.SetScaledDifferenceVector( 0.5f, MaximumPositionVector, MinimumPositionVector );
    }

    // ~~

    const BOOL_8 ContainsPositionVector(
        const REAL_32_VECTOR_3 & position_vector
        ) const
    {
        return
            !IsEmpty
